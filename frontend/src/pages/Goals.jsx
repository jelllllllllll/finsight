  ...
  return (
    <div className="container">
      <header className="top">
        <h1>Goals</h1>
        <nav><Link to="/dashboard">Dashboard</Link> | <Link to="/transactions">Transactions</Link></nav>
      </header>
      ...
      <section className="card">
        ...
        <form onSubmit={handleAdd} className="form-inline">
          ...
          <input value={form.targetAmount} onChange={e=>setForm({...form, targetAmount: e.target.value})} placeholder="Target amount" />
      ...
      <section className="card">
        ...
        <ul>
          {goals.map(g=>(
            <li key={g._id}>
              <strong>{g.name}</strong> — {g.currentAmount}/{g.targetAmount} — {g.status}
